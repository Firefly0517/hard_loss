...
def parse_arguments():
    ...
    parser.add_argument('--method', type=str, help='loss function',
                        choices=['supcon', 'yaware', 'threshold', 'expw', 'hardaware'], default='supcon')
...
def load_data(opts):
    ...
    train_loader_score = torch.utils.data.DataLoader(
        OpenBHB(opts.data_dir, train=True, internal=True, transform=T_train, label=opts.label),
...
if __name__ == '__main__':
    ...
    for epoch in range(1, opts.epochs + 1):
        ...
        if epoch % opts.save_freq == 0:
            ...
            mae_train, mae_int, mae_ext = compute_age_mae(model, train_loader_score, test_loader_int, test_loader_ext,
                                                          opts)
            ...
            ba_train, ba_int, ba_ext = compute_site_ba(model, train_loader_score, test_loader_int, test_loader_ext,
                                                       opts)
