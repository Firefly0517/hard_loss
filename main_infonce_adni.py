...
from data import FeatureExtractor, OpenBHB, bin_age, ADNI
...
def parse_arguments():
    ...
    # Data
    parser.add_argument('--train_all', type=arg2bool, help='train on all dataset including validation (int+ext)',
                        default=True)
    ...
    parser.add_argument('--method', type=str, help='loss function',
                        choices=['supcon', 'yaware', 'threshold', 'expw', 'hardaware', 'mse'], default='supcon')
...
def load_data(opts):
    ...
    # 1) 构建完整 ADNI 数据集（不加 transform）
    full = ADNI.ADNIT1AgeDataset(
    ...
    print(f"[INFO] ADNI split: train={len(train_idx)}, val={len(val_idx)}, test={len(test_idx)}")

    # 2) 包一层 dataset，加不同 transform
    class _Wrap(torch.utils.data.Dataset):
        ...
        def __getitem__(self, i):
            ...
            # base 返回 torch.Tensor，但 transform 期望 numpy
            x = x.numpy()
            x = self.transform(x)

            return x, y, None   # 第三个占位，兼容你训练代码

    train_dataset = _Wrap(full, train_idx, T_train)
    val_dataset   = _Wrap(full, val_idx,   T_test)
    test_dataset  = _Wrap(full, test_idx,  T_test)
    ...
    test_internal = torch.utils.data.DataLoader(
        val_dataset,
        batch_size=opts.batch_size,
        shuffle=False,
        num_workers=8,
    ...
    test_external = torch.utils.data.DataLoader(
        test_dataset,
        batch_size=opts.batch_size,
        shuffle=False,
        num_workers=8,
...
def evaluate(model, train_loader, test_int_loader, test_ext_loader, opts, epoch, writer):
    ...
    # ADNI 没有 site，直接跳过 BA
    ba_train = ba_int = ba_ext = float("nan")
    challenge_metric = mae_ext  # 或者 mae_int / (mae_int+mae_ext)/2 你自己定

    print(f"Age MAE: train={mae_train:.4f}, val={mae_int:.4f}, test={mae_ext:.4f}")
    print(f"Challenge score (MAE only): {challenge_metric:.4f}\n")

    if writer is not None:
        writer.add_scalar("train/mae", mae_train, epoch)
        writer.add_scalar("val/mae", mae_int, epoch)
        writer.add_scalar("test/mae", mae_ext, epoch)
